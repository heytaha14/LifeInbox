...
import {
  ...
  BookOpenText, CircleDollarSign, Clock3, Download, FileText, Fingerprint, Gift, HelpCircle, Home, Image as ImageIcon, Inbox, Layers3,
...
import { makeDrafts, makeNoteDraft, seedItems, seedThreads, type ItemType, type LifeItem, type LifeThread } from "@/lib/lifeinbox";

type View = "today" | "inbox" | "notes" | "threads" | "ask" | "settings";
    ...
    <div ref={root} className="landing">
      ...
      <main>
        <section className="hero-section">
          ...
          <p data-hero>A private AI workspace that turns screenshots, PDFs, voice notes, receipts, and messy thoughts into organized next steps or permanent notes.</p>
        ...
        <section id="features" className="feature-section">
          ...
          <div className="feature-bento">
            ...
            <article className="feature-card notes-feature-card"><div className="feature-icon"><BookOpenText size={20} /></div><span>Permanent Notes</span><h3>Not everything is a task.</h3><p>Save ideas, instructions, addresses, research, and reference material in a calm Notes library. It stays until you delete it.</p><div className="landing-note-preview"><span>REFERENCE NOTE</span><b>Venue access &amp; parking</b><p>Use the east entrance after 6 PM. Guest parking is on level B2.</p></div></article>
        ...
        <section className="workflow-section">
          ...
          <div className="workflow-steps">
            ...
            <article><span>3</span><h3>Work from clarity</h3><p>Use Today, Inbox, permanent Notes, grounded Ask, and connected Threads to know what matters next.</p></article>
          </div>
        </section>
...
function Sidebar({ view, setView, user, demo, inboxCount, noteCount, onCapture, onLogout }: { view: View; setView: (v: View) => void; user: AuthUser; demo: boolean; inboxCount: number; noteCount: number; onCapture: () => void; onLogout: () => void }) {
  ...
  return <>
    <button className={`mobile-menu ${mobileOpen ? "open" : ""}`} onClick={() => setMobileOpen(true)} aria-label="Open menu" aria-expanded={mobileOpen}><Menu size={20} /></button>
    ...
    <aside className={`sidebar ${mobileOpen ? "open" : ""}`}>
      ...
      <button className="capture-button" onClick={onCapture} aria-label="New capture"><Plus size={18} /><span>New capture</span><kbd>C</kbd></button>
      <nav aria-label="App navigation">{links.map((link) => <button key={link.id} className={view === link.id ? "active" : ""} aria-label={link.label} aria-current={view === link.id ? "page" : undefined} onClick={() => choose(link.id)}><link.icon size={18} /><span>{link.label}</span>{link.badge && <i aria-hidden="true">{link.badge}</i>}</button>)}</nav>
...
function Topbar({ view, onCapture, query, setQuery }: { view: View; onCapture: () => void; query: string; setQuery: (q: string) => void }) {
  ...
  return <header className="app-topbar"><div><span>{labels[view]}</span></div><div className="top-actions">{(view === "inbox" || view === "notes") && <label className="top-search"><Search size={16} /><input aria-label={`Search ${view}`} placeholder={view === "notes" ? "Search your notes" : "Search your inbox"} value={query} onChange={(e) => setQuery(e.target.value)} /></label>}<button className="top-capture" onClick={onCapture}><Plus size={16} /> Capture</button></div></header>;
}
...
function TodayView({ items, name, briefing, onDone, onCapture, onOpen, onViewAll }: { items: LifeItem[]; name: string; briefing: string; onDone: (id: string) => void; onCapture: () => void; onOpen: (item: LifeItem) => void; onViewAll: () => void }) {
  ...
  const todayItems = actionableItems.filter((i) => i.status === "today" || (i.priority === "high" && i.status !== "done"));
  const completed = actionableItems.filter((item) => item.status === "done").length;
  const remaining = actionableItems.length - completed;
  ...
  return <div className="view today-view">
    ...
    <section className="briefing-card" data-animate><div className="briefing-copy"><span><Sparkles size={15} /> TODAY&apos;S BRIEFING</span><h2>{attention ? `${attention} ${attention === 1 ? "thing deserves" : "things deserve"} your attention.` : "A clear slate for today."}</h2><p>{briefing}</p><div>{actionableItems.length ? <button onClick={onViewAll}>Show me what to do first <ArrowRight size={15} /></button> : <button onClick={onCapture}>Capture your first action <ArrowRight size={15} /></button>}<small>Updated just now</small></div></div><div className="briefing-art"><span className="orb-main">{attention}</span><span /><span /><span /></div></section>
    <div className="today-grid">
      <section className="panel" data-animate><div className="panel-head"><div><span>Up next</span><small>{todayItems.length} active {todayItems.length === 1 ? "item" : "items"}</small></div>{actionableItems.length > 0 && <button onClick={onViewAll}>View all <ArrowRight size={14} /></button>}</div><div className="focus-list">{todayItems.length ? todayItems.map((item) => <LifeItemRow key={item.id} item={item} onDone={onDone} onOpen={onOpen} />) : <div className="empty-state compact"><span><Sparkles size={22} /></span><h3>Nothing urgent</h3><p>Capture a thought and LifeInbox will turn it into a clear next step.</p><button onClick={onCapture}>New capture</button></div>}</div></section>
...
function LifeItemRow({ item, onDone, onOpen, dense = false }: { item: LifeItem; onDone: (id: string) => void; onOpen: (item: LifeItem) => void; dense?: boolean }) {
  ...
  return <article className={`life-row ${dense ? "dense" : ""} ${item.status === "done" ? "completed" : ""} ${isNote ? "note-row" : ""}`}>
    ...
    <button className="row-main" onClick={() => onOpen(item)}><span className={`type-icon ${meta.color}`}><Icon size={16} /></span><div><b>{item.title}</b><p>{item.content || item.summary}</p><div className="row-meta">{isNote ? <span><Fingerprint size={12} /> Saved permanently</span> : <span className={item.priority === "high" ? "urgent" : ""}><Clock3 size={12} /> {item.dueLabel ?? "No date"}</span>}{item.threadName && <span><Tag size={12} /> {item.threadName}</span>}</div></div></button>
...
function InboxView({ items, query, onDone, onOpen, onCapture }: { items: LifeItem[]; query: string; onDone: (id: string) => void; onOpen: (item: LifeItem) => void; onCapture: () => void }) {
  const [filter, setFilter] = useState<"all" | Exclude<ItemType, "note"> | "done">("all");
  const actionableItems = items.filter((item) => item.type !== "note");
  ...
  return <div className="view inbox-view">
    ...
    <div className="filter-row" data-animate>{(["all", "task", "event", "expense", "done"] as const).map((value) => <button key={value} className={filter === value ? "active" : ""} onClick={() => setFilter(value)}>{value === "all" ? "Open" : value === "done" ? "Completed" : typeMeta[value].label}{(value === "all" || value === "done") && <span>{actionableItems.filter((item) => value === "done" ? item.status === "done" : item.status !== "done").length}</span>}</button>)}</div>
...
function NotesView({ items, query, onOpen, onNewNote }: { items: LifeItem[]; query: string; onOpen: (item: LifeItem) => void; onNewNote: () => void }) {
  const notes = items.filter((item) => item.type === "note").filter((item) => `${item.title} ${item.summary} ${item.content || ""} ${item.threadName || ""}`.toLowerCase().includes(query.toLowerCase()));
  return <div className="view notes-view">
    ...
    {notes.length ? <section className="notes-grid" data-animate>{notes.map((note) => <button className="note-card" key={note.id} onClick={() => onOpen(note)}><div className="note-card-top"><span><BookOpenText size={17} /></span><small>{new Date(note.createdAt).toLocaleDateString("en-IN", { day: "numeric", month: "short", year: "numeric" })}</small></div><h2>{note.title}</h2><p>{note.content || note.summary}</p><footer>{note.threadName ? <span><Layers3 size={12} /> {note.threadName}</span> : <span><Fingerprint size={12} /> {note.source}</span>}<ArrowRight size={15} /></footer></button>)}</section> : <section className="panel empty-state notes-empty" data-animate><span><BookOpenText size={24} /></span><h3>{query ? "No matching notes" : "Your Notes library is ready"}</h3><p>{query ? "Try another search." : "Keep an address, idea, instruction, recipe, research snippet, or anything else you want to remember."}</p>{!query && <button onClick={onNewNote}>Save your first note</button>}</section>}
  </div>;
}
...
function SettingsView({ demo, user, items, onLogout, onToast, onReset }: { demo: boolean; user: AuthUser; items: LifeItem[]; onLogout: () => void; onToast: (message: string) => void; onReset: () => void }) {
  ...
  return <div className="view settings-view"><div className="view-heading" data-animate><span className="date-label">YOUR WORKSPACE</span><h1>Settings</h1><p>Manage your preferences, usage, privacy, and plan.</p></div>
    <div className="settings-layout" data-animate><nav>{nav.map((entry) => <button key={entry.id} className={section === entry.id ? "active" : ""} onClick={() => setSection(entry.id)}><entry.icon size={16} /> {entry.label}</button>)}</nav><div className="settings-content">
      ...
      {section === "privacy" && <><section className="settings-section"><div><h2>Privacy & retention</h2><p>Control how long original uploaded files are kept.</p></div><label className="select-setting"><div><b>Original file retention</b><small>Approved actions and Notes stay until you delete them.</small></div><select value={retention} onChange={(e) => setRetention(e.target.value)}><option value="7">7 days</option><option value="30">30 days</option><option value="90">90 days</option><option value="0">Delete after processing</option></select></label><button className="export-card" onClick={() => void exportData()} disabled={exporting}><span>{exporting ? <LoaderCircle className="spin" size={19} /> : <Download size={19} />}</span><div><b>{exporting ? "Designing your report…" : "Export LifeInbox report"}</b><small>Branded PDF · summaries, priorities, threads, notes, and page numbers</small></div><ArrowRight size={16} /></button><button className="button button-small settings-save" onClick={() => void persistPrefs()}>Save retention</button></section><section className="settings-section danger-section"><div><h2>Account</h2><p>Manage this session or permanently remove your workspace.</p></div><div><button className="button button-ghost" onClick={onLogout}><LogOut size={15} /> Log out</button><button className="danger-button" onClick={() => void deleteWorkspace()}><Trash2 size={15} /> Delete workspace</button></div></section></>}
...
function CaptureModal({ user, initialIntent, onClose, onReview }: { user: AuthUser; initialIntent: CaptureIntent; onClose: () => void; onReview: (drafts: LifeItem[], original: string, fileName?: string, captureId?: string, warning?: string) => void }) {
  ...
  async function process() {
    ...
    try {
      ...
      if (isAppwriteConfigured && user.id !== "demo") {
        ...
        const now = new Date();
        const result = await askOrExtract("extract", { input: captureText, source, fileId, fileName: file?.name, mimeType: file?.type, captureIntent: intent, localDate: now.toLocaleDateString("en-CA"), timezone: Intl.DateTimeFormat().resolvedOptions().timeZone });
...
function ReviewModal({ drafts, original, fileName, threads, onChange, onClose, onApprove, onReject }: { drafts: LifeItem[]; original: string; fileName?: string; threads: LifeThread[]; onChange: (items: LifeItem[]) => void; onClose: () => void; onApprove: (items: LifeItem[]) => Promise<void>; onReject: () => void }) {
  ...
  return <div className="modal-scrim"><section className="review-modal" role="dialog" aria-modal="true" aria-labelledby="review-title">
...
function ItemDetail({ item, onClose, onDone, onDelete, onSnooze }: { item: LifeItem; onClose: () => void; onDone: (id: string) => void; onDelete: (id: string) => void; onSnooze: (id: string) => void }) {
  const meta = typeMeta[item.type]; const Icon = meta.icon;
  const isNote = item.type === "note";
  return <div className="drawer-scrim" onClick={onClose}><aside className={`item-drawer ${isNote ? "note-detail" : ""}`} onClick={(e) => e.stopPropagation()}><div className="item-drawer-head"><span className={`type-icon ${meta.color}`}><Icon size={18} /></span><button className="modal-close" onClick={onClose} aria-label="Close item details"><X size={18} /></button></div><span className="date-label">{isNote ? "PERMANENT NOTE" : `${meta.label.toUpperCase()} · ${item.confidence}% CONFIDENCE`}</span><h2>{item.title}</h2><p className="item-summary">{item.summary}</p>{isNote && <div className="note-full-content">{item.content || item.summary}</div>}<div className="detail-grid">{!isNote && <div><span><Clock3 size={14} /> When</span><b>{item.dueLabel ?? "No date"}</b></div>}{item.amount && <div><span><CircleDollarSign size={14} /> Amount</span><b>{item.amount}</b></div>}{item.location && <div><span><Tag size={14} /> Location</span><b>{item.location}</b></div>}<div><span><Layers3 size={14} /> Thread</span><b>{item.threadName ?? "Unsorted"}</b></div></div><div className="detail-source"><Fingerprint size={17} /><div><b>{isNote ? "Saved permanently" : `Captured from ${item.source}`}</b><small>{new Date(item.createdAt).toLocaleDateString("en-IN", { day: "numeric", month: "short", year: "numeric" })} · {item.source}</small></div></div><div className="drawer-actions">{isNote ? <button className="danger-button delete-note-button" onClick={() => { if (window.confirm(`Delete “${item.title}”? This note cannot be recovered.`)) { onDelete(item.id); onClose(); } }}><Trash2 size={15} /> Delete note</button> : <><button className="button" onClick={() => { onDone(item.id); onClose(); }}><Check size={15} /> {item.status === "done" ? "Move to inbox" : "Mark complete"}</button><button className="button button-ghost" onClick={() => { onSnooze(item.id); onClose(); }}><Clock3 size={15} /> Snooze</button><button className="danger-icon" aria-label={`Delete ${item.title}`} onClick={() => { if (window.confirm(`Delete “${item.title}”?`)) { onDelete(item.id); onClose(); } }}><Trash2 size={16} /></button></>}</div></aside></div>;
}
...
export function LifeInboxApp() {
  ...
  function toggleDone(id: string) { const current = items.find((item) => item.id === id); if (!current || current.type === "note") return; const status = current.status === "done" ? "inbox" : "done"; setItems((all) => all.map((item) => item.id === id ? { ...item, status } : item)); setToast(status === "done" ? "Nice! Item completed" : "Item moved back to the inbox"); if (!demo && isAppwriteConfigured) void updateLifeItem(id, { status }); }
  ...
  function deleteItem(id: string) { const removed = items.find((item) => item.id === id); setItems((all) => all.filter((item) => item.id !== id)); setToast(removed?.type === "note" ? "Note deleted" : "Item removed"); if (!demo && isAppwriteConfigured) void deleteLifeItem(id); }
  ...
  function beginCapture(intent: CaptureIntent) {
  ...
  return (
    <div ref={appRoot} className="app-shell">
      <Sidebar view={view} setView={setView} user={user} demo={demo} inboxCount={items.filter((item) => item.type !== "note" && item.status !== "done").length} noteCount={items.filter((item) => item.type === "note").length} onCapture={openCapture} onLogout={() => void logout()} />
      <main className="app-main">
        ...
        {view === "notes" && <NotesView items={items} query={query} onOpen={setSelected} onNewNote={openNoteCapture} />}
      ...
      {captureOpen && <CaptureModal user={user} initialIntent={captureIntent} onClose={() => setCaptureOpen(false)} onReview={(drafts, original, fileName, captureId, warning) => { setReview({ drafts, original, fileName, captureId }); setReviewPaused(false); setCaptureOpen(false); if (warning) setToast(warning); }} />}
